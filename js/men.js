...
const menProducts = async () => {
  ...
  renderProducts(menProductsArray, currentPage);
  renderPagination(menProductsArray);
  searchBox(menProductsArray);
  searchEnter(menProductsArray);
  expensiveProducts(menProductsArray);
  cheapProducts(menProductsArray);
  mostSaledProducts(menProductsArray);
  filterSection(menProductsArray);
  mobileFiltering(menProductsArray);
};
const searchBox = (allProducts) => {
...
const searchEnter = (menProductsArray) => {
  searchBoxInput.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      const searchValue = searchBoxInput.value.toLowerCase();

      const filteredProducts = menProductsArray.filter((item) =>
        item.title.toLowerCase().includes(searchValue)
      );
      currentPage = 1;
      productWrapper.innerHTML = "";
...
const expensiveProducts = (menProductsArray) => {
  expensiveCheckInput.addEventListener("change", () => {
    currentPage = 1;
    let filteredProducts;

    if (expensiveCheckInput.checked) {
      filteredProducts = menProductsArray.filter(
        (item) => item.price >= 2000000
      );
    } else {
      filteredProducts = menProductsArray;
    }
...
const cheapProducts = (menProductsArray) => {
...
const mostSaledProducts = (menProductsArray) => {
  mostSaledCheckInput.addEventListener("change", () => {
    currentPage = 1;
    let filteredProducts;
    if (mostSaledCheckInput.checked) {
      filteredProducts = menProductsArray.filter((item) => item.saled >= 80);
    } else {
      filteredProducts = menProductsArray;
    }
    renderProducts(filteredProducts, currentPage);
...
const filterSection = (menProductsArray) => {
  filterBar.forEach((filter) => {
    filter.addEventListener("click", (event) => {
      ...
      switch (dataSetValue) {
        case "all":
          renderProducts(menProductsArray, 1);
          renderPagination(menProductsArray);
          break;

        case "expensive":
          const expensive = menProductsArray.filter(
            (item) => item.price >= 2000000
          );
        ...
        case "cheap":
          const cheap = menProductsArray.filter((item) => item.price < 2000000);
          renderProducts(cheap, 1);
          renderPagination(cheap);
          break;
        case "moastsaled":
          const moastsaled = menProductsArray.filter(
            (item) => item.saled >= 80
          );
...
const mobileFiltering = (menProductsArray) => {
...
const addInUserBasket = (productId) => {
  const mainProduct = menProductsArray.find(
